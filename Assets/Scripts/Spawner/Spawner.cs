...
public class Spawner : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        for (int i = 0; i < _repeatCount; i++)
        {
            ...
            MoveSpawner(_distanceBetweenRandomLine);
            GenerateBonus(_bonusSpawnPoints, _bonusTemplate.gameObject, _bonusSpawnChance);
        }
    }
    ...
    private void GenerateBonus(BonusSpawnPoint[] spawnPoints, GameObject generatedElement, int spawnChance)
    {
        for (int i = 0; i < spawnPoints.Length; i++)
        {
            if(Random.Range(0, 10) < spawnChance)
